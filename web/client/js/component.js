	/*! Reef v11.0.1 | (c) 2021 Chris Ferdinandi (Discontinued) | 2022 Maintained by Chris Mansbridge */
	...
	var Component = (function () {
		...
		function emit (type, detail = {}, elem = document) {
			...
			// Create a new event
			let event = new CustomEvent(`component:${type}`, {
				bubbles: true,
				cancelable: true,
				detail: detail
			});
		...
		function handler (instance) {
			return {
				...
				set: function (obj, prop, value) {
					...
					obj[prop] = value;
					instance._reactive && render(instance);
					return true;
				},
		 ...
		 * @param  {Contructor} instance The current Component instantiation
		...
		function clear_events(containor_el){
			//loop through all the events

			const old_events = events;
			const new_events = {};
			const non_view_listeners = ['home', 'alerts', 'selling_days', 'user_tools'];

			for (let event in old_events) {
				//loop through all the listeners

				for (let listener of old_events[event]) {
					//remove the listener
					//console.log(listener.elem.closest("main"),listener.elem, listener.instance)
					//check if the listener.elem is an html element contained within the main element
					if (non_view_listeners.includes(listener.instance.name)) {
						//if it is, add it to the new_events
						if (!new_events[event]) new_events[event] = [];
						new_events[event].push(listener);
					}
				}
			}
		...
		// 			//check if the listener is attached to the current element
		// 			if (containor_el === listener.elem || containor_el.contains(listener.elem){
		// 				//remove the event listener
		// 				listener.elem.removeEventListener(type, listener.callback, true);
		// 				//remove the listener from the events object
		// 				events[type] = events[type].filter(function (event) {
		...
		function getListener (type, elem, callback) {
			return events[type].find(function (listener) {
				//console.log(elem, listener.elem)
				return (elem === listener.elem) && 
					(callback.toString().replace(/\s/g, "") === listener.callback.toString().replace(/\s/g, ""));
			});
		}
		 ...
		 * @param {Constructor} instance The Component instance the element is in
		 */
		function addEvent (elem, name, value, instance) {
			...
			if (getListener(type, elem, callback)){
				return;
			}
			//console.log(type, elem, callback, instance, value)
		 ...
		 * @param  {Constructor} instance The Component instance the elements are in
		 */
		...
		function addAttribute (elem, att, val) {
			//console.log("Adding attribute", att, val);
		...
		function diffAttributes (template, existing, instance) {
			...
			for (let {name, value} of templateAtts) {
				...
				if (name.startsWith('on')) {
					//console.log("Adding event to an existing element")
					addEvent(existing, name, value, instance);
					continue;
				}
				//console.log(existing, name, value, "adding...")
		...
		function addDefaultAtts (elem, instance) {
			...
			for (let {name, value} of elem.attributes) {
				...
				// If its an event handler, maybe add it
				if (name.startsWith('on')) {
					//console.log("Adding event to newly created element");
		...
		function Constructor (elem, options = {}) {
			...
			let {data, store, template, isStore, setters, listeners, reactive = false} = options;
			...
			Object.defineProperties(this, {
				...
				query_options: {
					value: {
						...
						active_page: null,
						direction: "asc",
						//these 3 are essentialy filters and will be replace by filterx
						ilike: "",
						filter_by: null,
						row_status: "active",
						//contains the data needed to filter rows. There are two types, client filters and enforced model filters
						//this contains
						filterx: {
							is_active: true
						},
		...
		/**
		 * Dispatch an events to signal loading state
		 * @param {Boolean} state The loading state
		 */
		Constructor.prototype.loading = async function (state) {
		...
		Constructor.prototype.render = function () {
			...
			try {
				...
				Q('main').innerHTML = html`<h1 class="ta--c c--red m--800">Error rendering view, see console for details</h1>`;
				app.view.change('error', { error});
			}

		};

		/** 
		 * Clear all the event listners in events object that are attached to the current <main> element
		 * {
		"click": [
			{
				"elem": {},
		...
		// View Events in Components Closure
		Constructor.get_events = get_events;

		Constructor.clear_events = clear_events;
	...
	function use_effect_on_render(component, callback, first_render_only = true){
		component.elem.addEventListener('component:render', callback, { once: first_render_only });
	}


	const auth = new Component('#menu-viewer-component',{
		data: {
			mode: null,
			form: null
		},
		template: ({ form, mode }) =>  {
			switch (mode) {
				case "register":     
					return html`
						<section class="auth-register d--f fd--c ai--c jc--sb">
							<div class="auth-register">
								<h1 class="auth-header tt--c">${form.header}</h3>
								<form class="auth-form d--f fd--c g--xl ai--c" onsubmit="return false;">
									${form.fields.map(( f, i ) => { 
										const r_i = form.fields.length - i;
										return create_field(f, r_i);
									})}
									<button id="auth-submit" onclick="submit()" type="submit">Register</button>
								</form>
							</div>
						</section>`
				case "login":     
					return html`
						<section class="auth-login d--f fd--c ai--c jc--sb">
							<div class="auth-login">
								<h1 class="auth-header">${form.header}</h3>
								<form class="auth-form d--f fd--c g--xl ai--c" onsubmit="return false;">
									${form.fields.map((f, i) => create_field(f, i))}
									<button id="auth-submit" onclick="submit()" type="submit">login</button>
								</form>
							</div>
						</section>`
				case "about":
					return html`
						<div class="menu-views d--f fd--c ai--c jc--c">
							<p>Our mission is to provide investors with access and insight
							into publicly traded companies insider activities</p>
						</div>`;
				case "advertise":
					return html`
						<div class="menu-views d--f fd--c ai--c jc--c g--xl">
							<h1>Advertise with Insider Analysis</h1>
							<p>For inquiries please contact advertising@insider-analysis.com.</p>
						</div>`;
				case "contact":
					return html`
						<div class="menu-views d--f fd--c ai--c jc--c g--xl">
							<h1>Contact Insider Analysis</h1>
							<p>For inquiries please contact info@insider-analysis.com.</p>
						</div>`;
				case "membership":
					return html`
						<div class="menu-views d--f fd--c ai--c jc--c g--xl">
							<h1>Consider supporting through Patreon</h1>
							<ul style="list-style: none;">
								<li>- Support development of our analysis tools</li>
								<li>- Gain first access to unique analytics</li>
								<li>- Browse ad free</li>
							</ul>
						</div>`;
				default:
					break;
		...
		listeners: {
			submit: function(e){
				...
				const fd = new FormData(this.elem.querySelector("form"));
				const { mode } = this.data
				const that = this;
	
				arc.post(AUTH, mode === "login" ? LOGIN : REGISTER, fd)
					.then(({ data, status, message }) => {
						if (status === 1){
							show_errors(that, data.errors);
						} else {
							token.set(data.user.token);
							get_menu();
							doc.getElementById("menu-button").click();
						}
			...
			validate: e => listeners.validate(e)
		},
		setters: {
			load_form: function(props){
				console.log("Loading form", props)
				// this.data.form = load_form(props.mode);
			} 
		}
	});
